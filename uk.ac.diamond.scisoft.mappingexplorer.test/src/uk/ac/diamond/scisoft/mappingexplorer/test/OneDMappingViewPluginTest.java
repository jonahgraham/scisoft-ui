 ...
 */
public class OneDMappingViewPluginTest {
	...
	public final void testShowView() throws Exception {
		...
		op.run(null);
		window.getActivePage().showView(OneDMappingView.ID);

		IFile file = ResourcesPlugin.getWorkspace().getRoot().getProject("Test Project").getFolder("mappingexptest")
				.getFile("2495.nxs");
		window.getActivePage().openEditor(new FileEditorInput(file), HDF5TreeEditor.ID);

		PluginTestHelpers.delay(3000000);
	}

	private File getSourceDirectory() {
		return new File("/scratch/excalibur1/plugins/uk.ac.diamond.scisoft.mappingexplorer.test/testFiles/mappingexptest");
	}
