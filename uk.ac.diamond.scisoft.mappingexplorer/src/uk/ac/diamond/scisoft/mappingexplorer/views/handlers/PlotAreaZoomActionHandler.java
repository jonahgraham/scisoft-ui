...
/**
 * 
 * Action handler for Plot area zoom action when the view is
 * {@link IDatasetPlotterContainingView}
 * 
 * @author rsr31645
 * 
 */
public class PlotAreaZoomActionHandler extends AbstractHandler {
	private static final String PLOT_ZOOM_REGION_COMMAND_ID = "uk.ac.diamond.scisoft.analysis.rcp.PlotRegionZoomAction";
		...
		if (isEnabled) {
			...
			for (ToolItem toolItem : toolBar.getItems()) {
				...
				if (data instanceof CommandContributionItem) {
					...
					if (PLOT_ZOOM_REGION_COMMAND_ID.equals(cmdContribItem
							.getCommand().getId())) {
			...
			if (toolItemDisabled != null) {
				if (service instanceof IHandlerService) {
					...
					try {
						hs.executeCommand(PLOT_ZOOM_REGION_COMMAND_ID, event);
