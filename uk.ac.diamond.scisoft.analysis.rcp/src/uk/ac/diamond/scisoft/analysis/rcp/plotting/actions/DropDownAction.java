...
/*
 * KIELER - Kiel Integrated Environment for Layout Eclipse RichClient http://www.informatik.uni-kiel.de/rtsys/kieler/
 * Copyright 2009 by + Christian-Albrechts-University of Kiel + Department of Computer Science + Real-Time and Embedded
 * Systems Group This code is provided under the terms of the Eclipse Public License (EPL). See the file epl-v10.html
...
/**
 * The Class DropDownAction. This is a helper class that simplifies the creation of drop down actions, by adding simple
 * Actions. A DropDownAction always has a default Action which it refers to in the standard behaviour. It will always be
 * displayed first in the list. The list can not only contain Actions but also Separators for example.
 * 
...
public class DropDownAction extends Action implements IMenuCreator {
	...
	/**
	 * Instantiates a new drop down action. This always has a default Action that is the first action in the drop down
	 * menu. The DropDownAction inherits the icon, action, text, enableness and the visibility from this default Action.
	 * 
	...
	/**
	 * The listener interface for receiving propertyChange events. The class that is interested in processing a
	 * propertyChange event implements this interface, and the object created with that class is registered with a
	 * component using the component's <code>addPropertyChangeListener</code> method. When the propertyChange event
