...
public class InitPlotServer implements IStartup, ServerPortListener {
	...
	public void earlyStartup() {
		...
		if (AnalysisRCPActivator.isGDA()) return;
		...
		// If the RMI server has been vetoed, don't start it up, this also has issues
		//if (Boolean.getBoolean("uk.ac.diamond.scisoft.analysis.analysisrpcserverprovider.disable") == false) {
		final int analysisPort = AnalysisRpcAndRmiPreferencePage.getAnalysisRpcPort();
		if (analysisPort >= 1 && analysisPort <= 65535 && NetUtils.isPortFree(analysisPort)) {
			AnalysisRpcServerProvider.getInstance().setPort(analysisPort);
		}
