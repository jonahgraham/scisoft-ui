 ...
 */
public class ASCIILazyDatasetSaver implements IFileSaver {
	 ...
	 * @param fileName
	 */
	public ASCIILazyDatasetSaver(String fileName) {
	...
	private void writeDataset(String heading, ILazyDataset ds) throws IOException {
		wl("Dataset," + heading + "," + ds.getName());
		int[] shape = ds.getShape();
		for (int d : shape) {
			wl(Integer.toString(d));
	...
	public void saveFile(ILazyDataset lazyDataset) throws ScanFileHolderException {
		try {
			out = new BufferedWriter(new FileWriter(fileName));

			String heading = lazyDataset.getName();
			writeDataset(heading, lazyDataset);
			out.close();
